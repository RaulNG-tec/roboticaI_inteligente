...
def audio_filter():
    ...
    # Normalice audio to be beetwen -1 to 1
    sound = sound / 2.0**15
    ...
    for i, f in enumerate(freq):
        if f > 2400 and f < 2600:
            fft_spectrum[i] = 0.0
        elif f > 5900 and f < 6100:
            fft_spectrum[i] = 0.0
        elif f > 7900 and f < 8100:
            fft_spectrum[i] = 0.0
    ...
    wavfile.write("NoisyAudio.wav", sampFreq, signal)
    wavfile.write("NoiselessAudio.wav", sampFreq, noiseless_signal)
    playsound("NoisyAudio.wav")
    playsound("NoiselessAudio.wav")
